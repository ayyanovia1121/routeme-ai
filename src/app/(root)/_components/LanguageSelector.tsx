...
function LanguageSelector({ hasAccess }: { hasAccess: boolean }) {
  ...
  // get the current language and setLanguage function from the store
  const { language, setLanguage } = useCodeEditorStore();

  // useRef used to get the dropdown element to calculate its position and size
  const dropDownRef = useRef<HTMLDivElement>(null);

  // get the current language selector from the language config
  const currenLanguageSelector = Language_Config[language];
    ...
    <div className="relative" ref={dropDownRef}>
      ...
      >
        ...
        <div className="size-6 rounded-md bg-gray-800/50 p-0.5 group-hover:scale-110 transition-transform">
          <Image
            src={currenLanguageSelector.logoPath}
            alt={currenLanguageSelector.label}
            width={24}
            height={24}
        ...
        {isOpen && (
          ...
          >
            ...
            <div className="max-h-[280px] overflow-y-auto overflow-x-hidden">
              {Object.values(Language_Config).map((langItem, index) => {
                const isLocked = !hasAccess && langItem.id !== "javascript";
                ...
                return (
                  ...
                  >
                    <button
                      ...
                      ${language === langItem.id ? "bg-blue-500/10 text-blue-400" : "text-gray-300"}
                    ...
                    >
                      ...
                      <div
                        className={`
                         ...
                         ${language === langItem.id ? "bg-blue-500/10" : "bg-gray-800/50"}
                       `}
                      >
                        <div
                        ...
                        <Image
                          width={24}
                          height={24}
                          src={langItem.logoPath}
                          alt={`${langItem.label} logo`}
                          className="w-full h-full object-contain relative z-10"
                        />
                      </div>
